...
def f0_metrics(reference: np.ndarray, candidate: np.ndarray, sr: int) -> dict:
    ...
    f0_ref, _, _ = librosa.pyin(reference, **kwargs)
    f0_cand, _, _ = librosa.pyin(candidate, **kwargs)
    ...
    if mask.sum() < 5:
        return {
            "f0_corr": np.nan,
            "cent_rmse": np.nan,
            "uv_mismatch": float(1.0 - mask.mean()),
        }
    ...
    return {
        ...
        "uv_mismatch": float(
            np.mean(np.isfinite(f0_ref[:n]) != np.isfinite(f0_cand[:n]))
        ),
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Minimal audio metrics for Mosaic-SVC A/B/C/D comparisons."
    )
    parser.add_argument(
        "--reference", required=True, help="Source vocal for F0-retention comparison."
    )
