...
from mosaic_svc.p0.f0_dit_adapter import (
    F0DiTAdapterConfig,
    install_f0_dit_adapter,
    save_f0_dit_adapter,
)
...
def _set_source_schedule(adapter, wrapper, f0: torch.Tensor, frames: int, prompt_frames: int) -> None:
    schedule = adapter.schedule(f0, frames)
    schedule[:, :prompt_frames] = 0.0
    wrapper.set_schedule(schedule)


@torch.no_grad()
def _validate(args, model, adapter, wrapper, probe, semantic_fn, f0_fn, mel_fn, sr, device, paths, styles):
    adapter.eval()
    ...
    for path_index, path in enumerate(paths):
        ...
        for style_index, style in enumerate(styles):
            ...
            with torch.autocast(device_type=device.type, dtype=torch.float16 if args.fp16 else torch.float32):
                ...
                pitch = _pitch_loss(probe, predicted_mel, labels, prompt_frames, args.unvoiced_weight)
            reconstruction_losses.append(float(reconstruction.cpu()))
            pitch_losses.append(float(pitch.cpu()))
    wrapper.set_schedule(None)
    adapter.train()
    return (
...
def run(args: argparse.Namespace) -> Path:
    ...
    adapter, wrapper = install_f0_dit_adapter(
        model,
        config=F0DiTAdapterConfig(
            rank=args.rank,
    ...
    best_path = output_dir / "f0_dit_adapter_best.pt"
    stale = 0
    ...
    for step in tqdm(range(1, args.steps + 1)):
        ...
        with torch.autocast(device_type=device.type, dtype=torch.float16 if args.fp16 else torch.float32):
            ...
            reconstruction, predicted_mel = model.cfm(mel, lengths, prompt_len, condition, style)
            pitch = _pitch_loss(probe, predicted_mel, labels, prompt_frames, args.unvoiced_weight)
            pitch_weight = args.pitch_loss_weight * min(1.0, step / max(1, args.pitch_warmup_steps))
        ...
        if step % args.validate_every == 0 or step == args.steps:
            ...
            checkpoint = output_dir / f"f0_dit_adapter_step_{step:06d}.pt"
            metadata = {"step": step, "selection_objective": objective}
            save_f0_dit_adapter(adapter, checkpoint, metadata)
    ...
    print(f"Saved best F0 DiT adapter: {best_path}")
    return best_path


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train a direct DiT F0 adapter.")
