...
from mosaic_svc.p0.style_adapter import StyleAdapterConfig, install_style_slice_adapter, save_style_adapter
...
def run(args: argparse.Namespace) -> Path:
    ...
    for step in tqdm(range(1, args.steps + 1)):
        ...
        with torch.no_grad():
            ...
            prompt_len = torch.LongTensor([min(int(common * args.prompt_ratio), common - 1)]).to(device)
        ...
        if step % args.save_every == 0:
            save_style_adapter(adapter, Path(args.output) / f"style_adapter_step_{step:06d}.pt")

    out = Path(args.output) / "style_adapter_final.pt"
    save_style_adapter(adapter, out)
    print(f"Saved style adapter: {out}")
    return out


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train only Mosaic-SVC P0 Style-Slice Adapter.")
    ...
    parser.add_argument("--canonical-seconds", type=float, default=25.0)
