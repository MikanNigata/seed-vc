...
def _predict(converter, head, item, style):
...
@torch.no_grad()
def _validate(converter, head, paths, style, device, frames):
    head.eval()
    values = []
    for path in paths:
        prediction, target = _predict(converter, head, _load_item(path, device, frames, False), style)
...
def run(args):
    ...
    for step in tqdm(range(1, args.steps + 1)):
        ...
        with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=args.fp16 and device.type == "cuda"):
            prediction, target = _predict(converter, head, item, style)
        ...
        if step % args.validate_every == 0 or step == args.steps:
            validation = _validate(converter, head, validation_paths, style, device, args.segment_frames)
