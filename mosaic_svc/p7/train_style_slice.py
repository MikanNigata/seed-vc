...
def run(args: argparse.Namespace) -> Path:
    ...
    best_path = output_dir / "style_adapter_best.pt"
    stale_validations = 0
    ...
    for step in tqdm(range(1, args.steps + 1)):
        ...
        if step % args.validate_every == 0 or step == args.steps:
            validation = _validate(
                args, model, semantic_fn, f0_fn, mel_fn, sr, device, validation_paths, styles
            )
            train_loss = sum(recent_losses) / len(recent_losses)
            checkpoint = output_dir / f"style_adapter_step_{step:06d}.pt"
    ...
    print(f"Saved best style adapter: {best_path}")
    return best_path


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train a style-only residual adapter over frozen Seed-VC.")
